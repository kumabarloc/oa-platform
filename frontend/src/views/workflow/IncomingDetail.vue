  ...
  <el-card>
    ...
    <div v-loading="loading">
      <el-descriptions :column="2" border v-if="detail">
        ...
        <el-descriptions-item label="优先级"><el-tag :type="detail.priority==='high'?'danger':detail.priority==='low'?'info':'warning'" size="small">{{ {high:'紧急',normal:'普通',low:'低'}[detail.priority] }}</el-tag></el-descriptions-item>
        <el-descriptions-item label="状态"><el-tag>{{ statusName(detail.status) }}</el-tag></el-descriptions-item>
...
const loading = ref(false), detail = ref(null)
function statusName(s){return {received:'已收文',distributing:'分发中',processed:'已处理',archived:'已归档'}[s]||s}
onMounted(async(){loading.value=true;try{detail.value=await getIncomingDetail(route.params.id)}catch{}finally{loading.value=false}})
</script>
